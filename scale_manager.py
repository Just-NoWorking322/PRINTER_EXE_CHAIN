...
Для exe: короткий timeout чтения (0.05 с) и разовый reset_input_buffer() после open — см. _run_loop.
Лог scale_com.log — папка с exe (рядом с NurMarketKassa.sqlite3); отключить: DESKTOP_MARKET_SCALE_NO_FILE_LOG=1.
"""
    ...
    def _run_loop(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.debug("scale reset_input_buffer: %s", ex)
        ...
        while not self._stop.is_set():
            try:
                ...
                if self._stop.is_set():
                    break
                logger.warning("Весы COM: %s", e)
                _append_scale_log(f"SerialException: {e}")
                self._set_status(f"Ошибка порта: {e}")
                time.sleep(0.3)
            except Exception as e:
                if self._stop.is_set():
                    break
